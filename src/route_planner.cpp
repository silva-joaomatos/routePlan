...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    float gcurrent = current_node->g_value;
    ...
    for (int i = 0; i < current_node->neighbors.size(); i++) {
        ...
        neighbor->g_value = gcurrent + neighbor->distance(*current_node);
...
float Compare(RouteModel::Node* X, RouteModel::Node* Y) {
  float f1 = X->g_value + X->h_value; // f1 = g1 + h1
  float f2 = Y->g_value + Y->h_value; // f2 = g2 + h2
  return f1 > f2; 
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // loop the parens of each node until start node is reached
    ...
    //add start node
    path_found.push_back(*current_node);
    std::reverse(path_found.begin(), path_found.end()); //reverse the order so begin-->end
